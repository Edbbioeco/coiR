...
As previously made, we can also do for multiple images, making a function in `purrr::map_dbl()` function, to flat output to a numeric vector.
...
Additionally, we can set all COI values into a vector, to make a dataframe. We use `purrr::map_dbl()` to flat outputs to a numeric vector, named by `vector_index`. For more values details, we set `round = 3` argument in `coiR::coir_index()` functions, to set decimal places.

:::{.callout-note}
A good practice to assist loops for images in coiR package is store them into a contained folder, where images file names got a pattern.
:::
...
:::{.callout-note}
That data frame can be exported for different file formats, such as .xlsx or .csv. That practice assists save analysis results.
:::

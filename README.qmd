...
As previously made, we can also do for multiple images, making a function in `purrr::map_dbl()` function.
...
Finally, we make a data rame with those values, using `purrr::imap_dfr()`, to get image names.
